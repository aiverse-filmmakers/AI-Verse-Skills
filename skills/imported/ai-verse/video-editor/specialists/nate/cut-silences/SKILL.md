...
`{ words: [{ text, start, end }] }`. Obtain or normalize it through the active AI-Verse transcription provider before running this deterministic specialist.

## Usage

```bash
# 1) Plan only — compute the cut, write EDL + re-timed transcript (no video touched)
node scripts/cut-silences.mjs <transcript.json> \
...
Pass `<stem>.silence-transcript.json` (and the silenced video if rendered) to the package-local **cut-mistakes** specialist. Timestamps are already on the edited timeline and remain the downstream source of truth.


## AI-Verse Adaptation Notes

The executable algorithm in `scripts/cut-silences.mjs` is preserved byte-for-byte from Nate Herk's pinned source at `b1afdb1dcbcad39dd27638ea699f132fe44ce6df`.

Only workspace-specific invocation/transcription instructions were adapted. The silence thresholds, natural-breath policy, EDL structure, transcript retiming, FFmpeg trim/atrim/concat behavior and dry-run/apply semantics are unchanged.
