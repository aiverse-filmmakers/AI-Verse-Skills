...
1. Read the scoped workspace guide, the AI-Verse Video Editor ownership/routing contract, the package-local storytelling specialist, and the accepted HyperFrames provider before composition work. Use the user's brief over aesthetic defaults. Keep project artifacts in an explicit project workspace and preserve originals.
2. Probe the supplied source and reference: duration, dimensions, frame rate, audio, and existing transcript identity. Preserve originals. Reuse a matching word-level transcript or route through the active AI-Verse transcription provider, then normalize word timestamps before downstream editing. Check credentials, local dependencies, upload boundaries, and service costs before any external transcription call. Never substitute an older take.
...
1. Kie.ai was Nate's optional provider for generated moving inserts and images. In AI-Verse, use the user's selected/available generation provider or supplied assets; check authorization, credits, data-sharing boundaries, and the current provider contract before generating. Save model, prompt, task ID, selected output, and generation state; reuse successful jobs rather than resubmitting blindly.
...
6. Run `npx hyperframes lint` and current `npx hyperframes check`, then inspect the live Studio/preview before rendering. Local automated review is permitted when the user authorized autonomous iteration. Render a draft, inspect it, revise, then render final quality. If browser/render capture fails, diagnose with the canonical HyperFrames doctor/browser path and use supported Docker/cloud rendering where available; do not invent a substitute rasterizer or silently switch rendering systems. Await local fonts before capture. Normalize video inputs to a compatible frame-time base where needed and inspect encoded cuts for one-frame mismatches. Build final face crops from the native-resolution source rather than a downscaled editing proxy.
...
```powershell
node scripts/validate-plan.mjs video-projects/<slug>
node scripts/validate-footage.mjs video-projects/<slug>
```
...
Adapted from Nate Herk's MIT-licensed `short-form-edit` Skill at `b1afdb1dcbcad39dd27638ea699f132fe44ce6df`.

Preserved:
- editorial hook/open-loop/payoff methodology;
- full-reference analysis rather than cherry-picked frames;
- footage ledger/provenance discipline;
- transcript-word anchoring;
- B-roll, captions, sound-design and premium-motion quality gates;
- exact plan/footage validator implementations.

Adapted:
- removed Nate-workspace-specific API/tool setup references;
- provider selection routes through AI-Verse runtime/provider authority;
- old renderer-fallback guidance was replaced by the accepted HyperFrames 0.8.40 doctor/browser/Docker/cloud path;
- validator invocation is package-local.
