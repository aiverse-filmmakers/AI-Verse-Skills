...
```bash
node scripts/find-cut-candidates.mjs \
...
Writes the EDL, a re-timed `<stem>.mistakes-transcript.json` for downstream editorial/motion timing, a decisions log, and, with `--apply`, the cut video using FFmpeg trim+concat with A/V kept in sync. Drop `--apply` for a dry run.
...
The candidate detector and approved-cut renderer scripts are preserved byte-for-byte from Nate Herk's pinned source at `b1afdb1dcbcad39dd27638ea699f132fe44ce6df`.

The review gate remains mandatory: candidate detection proposes, editorial judgment approves, and only approved ranges may be applied. AI-Verse adapted only package-local paths and removed assumptions about Nate's repo-level validators.
