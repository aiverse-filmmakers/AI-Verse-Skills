...
- **Shader transition**: no exit animation needed — the shader handles the blend. Use the current canonical HyperFrames transition/registry guidance for shader setup and supported transition patterns. Do not copy stale provider-internal paths from the Nate baseline.
- **Hard cut**: no exit animation. The scene simply ends.

For transition implementations, use the current canonical HyperFrames provider guidance or approved registry item available in the installed provider generation.
...
- **Timeline readiness before registration** — build deterministic render-critical state before exposing the registered timeline as ready. Current HyperFrames may support async setup when registration occurs only after the build completes; never register an empty timeline early.
...
This reference is adapted from Nate's pinned source for HyperFrames 0.8.40.
...
- keep repeats finite;
- media playback/timing remains framework-owned;
- run `npx hyperframes lint` and `npx hyperframes check` after build changes.
