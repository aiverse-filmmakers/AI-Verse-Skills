...
**CSS transitions** (choose from the current canonical HyperFrames provider guidance or an approved project transition pattern):
...
**Shader transitions** (only when supported by the current provider/project and justified by the beat):
...
- Use the current canonical HyperFrames transition/registry guidance for any WebGL boilerplate. Do not copy stale provider-internal paths from the Nate baseline.
...
This storyboard method is preserved from Nate Herk's pinned source at `b1afdb1dcbcad39dd27638ea699f132fe44ce6df`.

The experience-first beat design, asset audit, per-beat world/mood/camera/animation/SFX
brief, depth-layer planning, and velocity-matched transition thinking are preserved.

Only provider-path assumptions were changed:

- do not reference Nate's old bundled `skills/hyperframes/...` tree;
- resolve transitions through the installed canonical HyperFrames 0.8.40 provider,
  approved registry items, or project-local transition patterns;
- provider/runtime correctness outranks any old transition snippet.
