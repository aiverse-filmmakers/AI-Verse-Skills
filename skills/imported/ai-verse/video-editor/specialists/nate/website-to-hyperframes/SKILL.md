...
**Read:** The canonical AI-Verse HyperFrames 0.8.40 provider guidance. Provider/runtime rules are authoritative.
...
Lint, check, preview, and inspect rendered evidence. Create a HANDOFF.md for multi-session continuity.

**Gate:** `npx hyperframes lint` and current `npx hyperframes check` pass, then preview/render evidence is inspected for the requested deliverable.
...
| [step-7-validate.md](references/step-7-validate.md)     | Step 7 — lint, check, preview/render QA, handoff                                                                                                                      |
...
Adapted from Nate Herk's MIT-licensed `website-to-hyperframes` Skill at `b1afdb1dcbcad39dd27638ea699f132fe44ce6df`.

Preserved:
- website capture and summary gate;
- DESIGN.md extraction;
- script-before-storyboard sequencing;
- beat-level creative direction;
- VO/transcript timing before composition;
- per-composition self-review;
- final handoff artifact.

Adapted:
- the canonical HyperFrames 0.8.40 provider replaces Nate's bundled `/hyperframes` Skill;
- `hyperframes validate` is replaced by current `hyperframes check`;
- final completion requires actual preview/render evidence rather than static composition source alone;
- Interface Designer may assist visual direction but does not take ownership of this video workflow.
