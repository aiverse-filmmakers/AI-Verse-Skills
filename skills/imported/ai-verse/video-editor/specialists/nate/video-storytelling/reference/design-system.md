...
is a suggestion. When the active project provides Nate's token QA helper, run it; otherwise enforce the same design-system constraint through the Video Editor visual QA review and do not claim the absent script ran.
...
When the project provides Nate's `qa-presence` helper, use it for speaker-presence measurement (Nate's reference baseline notes bare footage at 100.0%):
