...
- Use the current canonical HyperFrames worker/default render policy. Do not force `--workers 1` as a blanket rule: AI-Verse acceptance for HyperFrames 0.8.40 rendered nested video successfully with two workers. Reduce workers only when a reproduced project-specific issue justifies it.
...
Adapted from Nate Herk's MIT-licensed `video-storytelling` Skill at `b1afdb1dcbcad39dd27638ea699f132fe44ce6df`.

The persistent-world, spatial storytelling, spotlight hierarchy, camera-as-edit,
cover-seam and contiguous-frame review methodology is preserved. Repo-level QA
commands that were not part of Nate's Skill package were converted into conditional
project gates plus the canonical AI-Verse/HyperFrames verification contract.

The old blanket `--workers 1` requirement was removed because the accepted
HyperFrames 0.8.40 media test rendered nested video correctly with two workers.
